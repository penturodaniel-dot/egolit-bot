...
Для кожного варіанту напиши 1 коротке речення (10-20 слів) — чому він підходить.

СУВОРІ ПРАВИЛА:
1. Спирайся ТІЛЬКИ на факти з опису варіанту — нічого не вигадуй.
2. ЗАБОРОНЕНО писати "можливо", "якщо", "може бути", "припускаємо" — будь-яке припущення.
3. Якщо запит містить ім'я артиста/людини:
   - ім'я ТОЧНО збігається з назвою варіанту → напиши чому цей виконавець підходить
   - ім'я НЕ збігається з назвою варіанту → поверни порожній рядок ""
4. Якщо реальної причини відповідності немає → поверни порожній рядок ""

Відповідай ТІЛЬКИ валідним JSON-масивом рядків (стільки елементів скільки варіантів):
["причина 1", "", "причина 3", ...]"""
